...
function compose(w, h, message, lang) {
  ...
  for (let k = suggested; k >= 0.55; k -= 0.02) {
    ...
    // Distribute the slack. A tall canvas reads better with the block set high, the way
    // the comp does it; a wide one just looks bottom-empty unless the stack is centred.
    const slack = (h - pad * 2 - total);
    let y = pad + slack * (w / h > 1.2 ? 0.5 : 0.34);
